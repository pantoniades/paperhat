...
class TestDispatch:
    ...
    def test_select_station_refetches(self, app, mock_epd, sample_station, sample_arrivals):
        """SelectStation should re-fetch live arrivals."""
        app.mta.fetch.return_value = sample_arrivals
        app._dispatch(SelectStation(station=sample_station), mock_epd)

        app.mta.fetch.assert_called_once_with(sample_station)
        assert isinstance(app._screen, StationScreen)
...
class TestFullNavigation:
    def test_home_to_subway_to_station_and_back_twice(
        ...
        # Subway → Station (re-fetches live arrivals)
